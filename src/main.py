...
import random
import transformers
from transformers import AutoTokenizer, EarlyStoppingCallback, DataCollatorForSeq2Seq, GenerationConfig, TrainerCallback
...
class EpochTimeCallback(TrainerCallback):
    ...
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        print(f"Epoch {state.epoch:.0f} took: {duration:.2f}s")
...
def main(args):
    ...
    if args.use_time:
        time_bins = [int(bin) if bin != 'inf' else float('inf') for bin in args.time_bins.split(',')]
        temporal_tokens = [f'<[{left},{right})>' for left, right in zip(time_bins[:-1], time_bins[1:])]
        tokenizer.add_tokens(temporal_tokens)
    ...
    if args.init_new_tokens:
        ...
        if args.use_time:   
            temporal_token_embs = torch.load(os.path.join(args.data_path, args.datasets, 'temporal_token_emb.pt'))
            start_idx = tokenizer.get_vocab()[test_data.temporal_tokens[0]]
            end_idx = tokenizer.get_vocab()[test_data.temporal_tokens[-1]]
            model.shared.weight.data[start_idx:end_idx+1] = temporal_token_embs
