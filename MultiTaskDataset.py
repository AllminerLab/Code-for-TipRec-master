...
from utils.utils import ReadLineFromFile, load_item_info, check_item_content, flatten_attributes
...
class MultiTaskDataset(Dataset):
    def __init__(self, args, dataset, mode):
        ...
        sequence_file = os.path.join(self.data_path, self.dataset, 'sequential_data.txt')
        if not os.path.exists(sequence_file) and self.dataset.lower() in {'ml1m', 'ml-1m'}:
            fallback_files = ['user_sequence.txt', 'user_sequence_sequential_indexing_original.txt']
            for name in fallback_files:
                candidate = os.path.join(self.data_path, self.dataset, name)
                if os.path.exists(candidate):
                    sequence_file = candidate
                    break
        if not os.path.exists(sequence_file):
            raise FileNotFoundError(f"Cannot find sequential data file for {self.dataset}.")
        self.user_sequence = ReadLineFromFile(sequence_file)
        self.user_sequence_dict = indexing.construct_user_sequence_dict(self.user_sequence)
        # load user sequence timestamp data
        self.use_time = args.use_time
        if self.use_time:
            self.user_timestamp = ReadLineFromFile(os.path.join(self.data_path, self.dataset, 'sequential_time.txt'))
            self.user_timestamp_dict = indexing.construct_user_sequence_dict(self.user_timestamp)
            self.time_bins = [int(bin) if bin != 'inf' else float('inf') for bin in args.time_bins.split(',')]
            self.temporal_tokens = [f'<[{left},{right})>' for left, right in zip(self.time_bins[:-1], self.time_bins[1:])]
        ...
        #elif self.item_indexing == 'independent' and args.use_timestamp:
        ...
        if any(['content' in p for p in self.prompt_info]):
            ...
            if 'attributes' in self.use_info_key:
                for iid in self.item_info:
                    self.item_info[iid]['attributes'] = flatten_attributes(self.item_info[iid].get('attributes'))
    ...
    def load_train(self):
        ...
        for user in self.reindex_user_seq_dict:
            ...
            if self.use_time:
                timestamps = list(map(int, self.user_timestamp_dict[user][:-2]))
                time_diffs = np.diff(timestamps) / (24 * 3600)  # 转换为天
                temporal_token = pd.cut(time_diffs, bins=self.time_bins, labels=self.temporal_tokens, right=False)
                ...
                if 'history' in self.prompt_info:
                    ...
                    if self.prefix > 0:
                        ...
                        for j in range(len(history)):
                            history_with_gap.append(history[j])
                            if j < len(temporal_token):
                                history_with_gap.append(temporal_token[j])
                        one_sample['history'] = self.his_sep.join(history_with_gap)
                    else:
                    ...
                    for item_idx in items_with_content:
                        ...
                        for key in self.use_info_key:
                            if key in self.item_info[raw_idx]:
                                value = self.item_info[raw_idx][key]
                                if value in (None, ''):
                                    continue
                                if key == 'category':
    ...
    def load_validation(self):
        ...
        for user in sorted_reindex_user_seq_dict:
            ...
            if self.use_time:
                timestamps = list(map(int, self.user_timestamp_dict[user]))
                time_diffs = np.diff(timestamps) / (24 * 3600)  # 转换为天
                temporal_token = pd.cut(time_diffs, bins=self.time_bins, labels=self.temporal_tokens, right=False)
            one_sample = dict()
            one_sample['dataset'] = self.dataset
    ...
    def _construct_sentence_all(self):
        ...
        for task in self.tasks:
            for i in range(len(self.data_samples)):
                ...
                if self.his_sep == '':
                    history = re.findall(r"<item_\d+>", datapoint['history'])
                else:
